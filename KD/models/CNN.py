		...
		for layer in layers:
			if layer.startswith('Conv'):
				...
				if num_fc_layers_remained == 0:
					...
					self.embDim = previous_layer_size
				else:
...
def create_cnn_model(plane_size, num_classes):
	
    model_spec = plane_cifar10_book.get(str(plane_size)) if num_classes == 10 else plane_cifar100_book.get(str(plane_size))
    model = ConvNetMaker(model_spec)
