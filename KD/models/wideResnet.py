...
class WideResNet(nn.Module):
    ...
    def __init__(self, d, k, n_classes, input_features, output_features, strides,if_large=False):
        ...
        self.embDim = filters[-1]

    def forward(self, x, last=True, freeze=False, is_feat=False):
        ...
        if is_feat:
            return [attention0, attention1, attention2, attention3, out],self.fc(out)
        elif last:
                return self.fc(out), out
        return self.fc(out)
...
strides = [1, 1, 2, 2]
def WRN_16_X(depth,width,num_classes=10,input_features=3,if_large=False):
    #print(input_features)
    return WideResNet(d=depth, k=width, n_classes=num_classes, input_features=input_features, output_features=16, strides=strides,if_large=if_large)
