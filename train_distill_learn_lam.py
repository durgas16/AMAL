...
class TrainClassifier:
    ...
    def create_model(self, if_teacher=False,n_channels=3):
        ...
        if mtype == 'ResNet18':
        ...
        elif mtype == 'DenseNet121':
            model = DenseNet121(self.configdata['model']['numclasses'])
        elif mtype == 'ResNet50':
            ...
            if self.configdata['dataset']['name'] in ['cars','flowers','airplane','dogs','Cub2011']:
                ...
                model = WRN_16_X(depth=d, width =w,num_classes=self.configdata['model']['numclasses'],\
                input_features=n_channels,if_large=False)
    ...
    def train(self):
        ...
        if valid:
            ...
            if self.configdata['dataset']['feature'] == 'classimb':
                ...
                "organsmnist","octmnist","pneumoniamnist"]:
                    trainset, testset, num_cls,n_channels = load_dataset_custom(self.configdata['dataset']['datadir'],
                                                                               self.configdata['dataset']['name'],
                                                                               self.configdata['dataset']['feature'],
                                                                               valid=valid)
                    n_channels = 3
        ...
        self.configdata['model']['numclasses'] = num_cls
        ...
        #if self.configdata['ds_strategy']['type'] in ['LearnLam']:
        #    lambdas = torch.full((N,),_lambda, device=self.configdata['train_args']['device'],requires_grad=True)
        #    lam_optimizer, lam_scheduler = self.optimizer_with_scheduler([{'params': lambdas}],if_model=False)
        ...
        print(lambdas.max(), lambdas.min(), torch.median(lambdas), torch.quantile(lambdas, 0.75),
                                    torch.quantile(lambdas, 0.25))
        ...
        for i in range(start_epoch, self.configdata['train_args']['num_epochs']):
            ...
            if (i < self.configdata['ds_strategy']['warm_epoch']) or \
                ...
                for batch_idx, (inputs, targets,indices) in enumerate(trainloader_ind):
                    inputs, targets = inputs.to(self.configdata['train_args']['device']), targets.view(-1).to(
                    ...
                    if _lambda > 0:
                            ...
                            if self.configdata['ds_strategy']['type'] in ['LearnLam']:
                                ...
                                loss = torch.mean((1 - lambdas[indices]) * loss_SL + lambdas[indices]*loss_KD)
                                #print(loss_SL,loss_KD)
                            else:
                                loss_SL = criterion(outputs, targets)
                                loss_KD = nn.KLDivLoss(reduction='batchmean')(F.log_softmax(outputs / Temp, dim=1), \
                                                                          F.softmax(teacher_outputs / Temp, dim=1))
                                loss = (1 - _lambda) * loss_SL + _lambda * Temp * Temp * loss_KD
            ...
            elif self.configdata['ds_strategy']['type'] in ['RewLam','LearnLam','UpdateLam']:
                ...
                if (i % select == 0 or i in self.configdata['ds_strategy']['schedule']):
                    ...
                    if self.configdata['ds_strategy']['type'] in ['RewLam']:                    
                        ...
                        lambdas = rel.get_lambdas(optimizer.param_groups[0]['lr'])

                        print(lambdas.max(), lambdas.min(), torch.median(lambdas), torch.quantile(lambdas, 0.75),
                                torch.quantile(lambdas, 0.25))

                        if self.configdata['ds_strategy']['project']:
                            lambdas[lambdas<0.5] = 0.1
                            lambdas[lambdas>=0.5] = 0.9
                            print(lambdas.max(), lambdas.min(), torch.median(lambdas), torch.quantile(lambdas, 0.75),
                                    torch.quantile(lambdas, 0.25))

                    elif self.configdata['ds_strategy']['type'] in ['LearnLam']:
                        lambdas = lelam.get_lambdas(optimizer.param_groups[0]['lr'],lambdas)
                ...
                for batch_idx, (inputs, targets,indices) in enumerate(trainloader_ind):
                    ...
                    if self.configdata['ds_strategy']['type'] in ['UpdateLam']:
                        lambdas[indices] = lambdas[indices] - optimizer.param_groups[0]['lr']*(loss_KD - loss_SL)
            ...
            with torch.no_grad():
                for batch_idx, (inputs, targets,indices) in enumerate(trainloader_ind):
                    ...
                    inputs, targets = inputs.to(self.configdata['train_args']['device']), targets.view(-1).to(
                        self.configdata['train_args']['device'], non_blocking=True)
                    outputs = train_model(inputs)
                    target_trn[indices] = targets.cpu()
            ...
            np.save(os.path.join(all_logs_dir, 'targets.npy'),target_trn.numpy())
            ...
            if (i < self.configdata['ds_strategy']['warm_epoch']):
                all_lambda = []
            elif self.configdata['ds_strategy']['type'] in ['RewLam',"LearnLam"]:
                all_lambda.append(lambdas)
            ...
            if ((i + 1) % self.configdata['ckpt']['save_every'] == 0):
                ...
                if self.configdata['ckpt']['is_save'] == True:
                    ...
                    elif (i + 1) % 10 == 0 and (i + 1) != self.configdata['train_args']['num_epochs']:
                        self.save_ckpt({'state_dict': train_model.state_dict()}, \
                        os.path.join(ckpt_dir, 'model_'+str(i + 1)+'.pt'))
                    ...
                    if ((trainset.dataset[trainset.indices])[0]).shape[1] == 2:
                        ...
                        if i + 1 > 11 and self.configdata['ds_strategy']['type'] in ['RewLam','LearnLam']:
                            #fig, ax = plt.subplots()
